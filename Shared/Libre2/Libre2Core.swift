...
enum Libre2Core {
    ...
    static func word(_ high: UInt8, _ low: UInt8) -> UInt16 { UInt16(high) << 8 | UInt16(low) }
    static func readBits(_ data: Data, _ byte: Int, _ bit: Int, _ count: Int) -> Int {
        (0..<count).reduce(0) { result, i in
            let offset = byte * 8 + bit + i
            return result | (Int((data[offset / 8] >> (offset % 8)) & 1) << i)
        }
    }
    ...
    ///
    /// after removing that smoothing workflow, the remaining job here is only to align the next sparse 16-value BLE frame against the previous one,
    ...
    public static func decryptBLE(sensorUID: Data, data: Data) throws -> [UInt8] {
        ...
        guard crc16(Data(result.prefix(42))) == word(result[42], result[43]) else {
    ...
    // MARK: - BLE frame parsing

    /// - returns:
    ///     - array of Libre2Sample. Returns empty array if the latest value is 0.0 for any reason
    ...
    public static func parseBLEData( _ data: Data, libre1DerivedAlgorithmParameters : Libre2Calibration?, state: inout Libre2ParserState, now: Date = Date(), allowsRawFallback: Bool = true) -> (bleGlucose: [Libre2Sample], sensorTimeInMinutes: UInt16) {
        ...
        if let previousRawGlucoseValues = state.previousRawGlucoseValues {
            if Array(rawGlucoseValues.prefix(5)) == Array(previousRawGlucoseValues.prefix(5)) {
        ...
        if bleGlucose.count > 2 {
            for i in 1..<(bleGlucose.count - 1) where bleGlucose[i].glucoseLevelRaw == 0 && bleGlucose[i-1].glucoseLevelRaw > 0 && bleGlucose[i+1].glucoseLevelRaw > 0 {
                bleGlucose[i].glucoseLevelRaw = (bleGlucose[i-1].glucoseLevelRaw + bleGlucose[i+1].glucoseLevelRaw) / 2
            }
        }
    ...
    private static func appendPreviousValues(to rawGlucoseValues: inout [Int], rawTemperatureValues: inout [Int], temperatureAdjustmentValues: inout [Int], state: Libre2ParserState) {
