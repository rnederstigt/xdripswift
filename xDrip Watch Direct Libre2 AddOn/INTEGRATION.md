...
The refactor is based on the published prototype at `4913bb9`. The upstream comparison base remains `53b3d6bf` (7.0.0 build 4231). Most implementation lives behind add-on hooks. History synchronisation adds a ninth existing Swift integration point, the root coordinator, to refresh phone displays after durable imports.
...
- **Watch/Libre2WatchAddOn** validates and displays direct readings. Ordinary phone relay keeps the upstream handler and its acceptance rules. It uses `Libre2WatchDisplay` rather than a concrete Watch model.
- **Shared/Readings** contains the display pipeline and the separate history message, durable outbox and phone sensor registry, independently tested on the host.
- **Watch/Libre2WatchHistorySync** persists the newest real measurement before display and manages interactive/queued transfers with the existing WCSession.
- **iPhone/Libre2PhoneHistorySync** registers the original database sensor, imports repeat-safe batches and acknowledges only after saving to the persistent store.
...
| `xDrip/BluetoothTransmitter/CGM/Libre/Libre2/CGMLibre2Transmitter.swift` | Owns one sensor adapter and forwards construction, connection, unlock-write, real-reading and ordinary-NFC events. Asks the adapter to reserve an unlock counter before the existing payload/write code. Keeps the existing parser and test-data replay distinction. Connection, disconnection, failed connection and Bluetooth state callbacks keep the checklist live without polling. Ordinary NFC now asks the adapter to reset abandoned Direct Libre state; only reset scans use fresh credentials and the confirmed-disconnect barrier. |
...
| `xDrip/Managers/Watch/WatchManager.swift` | Routes handoff and history messages to the add-on, provides the existing database/session for imports and registration, and forwards connection events alongside the original update behavior. |
| `xDrip/Managers/Application/RootApplicationCoordinator.swift` | Observes successful history imports and refreshes phone charts, existing Watch relay and widgets. Does not invoke new-reading alarms or force the chart back to now. |
...
The refactor does not change the persisted journal/message keys, session schema, counter reservation order, disconnect barriers, reclaim semantics, glucose conversion formulas. The subsequent refresh optimization removes the add-on's periodic UI timers as described below. History synchronization subsequently adds an independent message schema and journals as described in [HISTORY_SYNC.md](HISTORY_SYNC.md). It adds no workout session, background execution mechanism, alarm system or sensor backfill. Existing installations do not need NFC solely because files moved; install matching phone/Watch builds and validate normal switching before relying on the new build.
...
## Ordinary NFC reset after deletion

[NFC_RESET.md](NFC_RESET.md) documents the subsequent recovery fix. The phone adapter and session store supersede abandoned Direct Libre state when an ordinary scan starts. `LibreNFC.swift` itself is unchanged by this fix; the caller supplies a fresh unlock code only when replacing experimental credentials.

The standalone Reclaim via NFC button and confirmation sheet have been removed. Recovery guidance now points to ordinary Libre Add/Connect scanning. Existing recovery records and verification retries remain compatible. The NFC reader reference is cleared after a scan only if that scan was a Direct Libre reset; the original ordinary-scan retry/reconnect behavior is preserved.

## Behavior-preservation cleanup

See [BEHAVIOR_ISOLATION.md](BEHAVIOR_ISOLATION.md) for the upstream relay comparison, scoped buffer resets, dormant diagnostics, retained counter safeguard and targeted NFC diagnostics. Run `python3 "xDrip Watch Direct Libre2 AddOn/Scripts/check_upstream_relay.py"` from the repository root to compare the actual relay handlers.
