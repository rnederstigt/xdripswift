...
final class Libre2CaptureController: ObservableObject {
    ...
    var description: String {
        ...
        if status.isRecording {
            return "Recording since \(status.startedAt.formatted(date: .omitted, time: .standard)); "
                + "ends by \(status.expiresAt.formatted(date: .omitted, time: .standard)). \(status.events) events at last refresh."
        }
    ...
    private func request(_ command: String, id: UUID? = nil, offset: Int? = nil,
        ...
        guard WCSession.default.activationState == .activated, WCSession.default.isReachable else {
            error = Texts_DirectLibre.captureNeedsWatch
            isBusy = false
            return
        }
        isBusy = true
        ...
        let token = UUID()
        requestID = token
        var message: [String: Any] = [Libre2ActivityLog.requestKey: true, Libre2DiagnosticCapture.commandKey: command]
