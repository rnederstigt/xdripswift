...
The purpose of this test is to investigate and potentially recover live phone delivery when background sensor collection continues but `WCSession.isReachable` remains false. In the recorded device run, the custom Watch notification callback was followed about 5 ms later by restored reachability; six subsequent readings arrived on the phone within roughly 0.1–0.4 seconds while both apps stayed backgrounded. The existing reachability callback resumes latest-reading delivery, independently of historical acknowledgements. This is an observed association with Watch notification presentation, not an explicit connection request from the notification controller or a confirmed explanation of watchOS scheduling. The action remains manual, with no automatic notification on handoff, repeated wake-up loop or guarantee that reachability persists. See [the user workflow](../README.md#if-watch-readings-update-but-the-phone-lags) and [recorded evidence](TESTING.md#recorded-evidence).
